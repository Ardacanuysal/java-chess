...
public class Frame extends JFrame {
    ...
    Satranc game = new Satranc();
    private final JPanel panel = new JPanel();
    private final JButton btnNew = new JButton("yeni oyun");
    private final JButton btnExit = new JButton("ÇIKIŞ");
    ...
    private final JLabel lblTurnblack = new JLabel("sıra ");
    private final JLabel lblPoweredByHalil = new JLabel("powered by Arda Can Uysal");
    ...
    private void initGUI() {
        ...
        for (int i = 0; i < stones.length; i++) {
            for (int j = 0; j < stones.length; j++) {
                ...
                if ((i + j) % 2 == 0) {
                    stones[i][j].setBackground(Color.blue);
                } else {
                    stones[i][j].setBackground(Color.gray);
                }
                stones[i][j].addActionListener(new ActionListener() {
                    ...
                    public void actionPerformed(ActionEvent arg0) {
                        ...
                        loop:
                        for (int j2 = 0; j2 < stones.length; j2++) {
    ...
    boolean clicks(int sx, int sy, int ax, int ay) {
        if (firstClick) {
            if ((color.contentEquals("Black") && (game.getBoard(sx, sy) > 0))
                ...
                sourceX = sx;
                sourceY = sy;
                stones[sx][sy].setBackground(Color.pink);
                temp = game.findCanMoveField(color, sx, sy);
                for (int i = 0; temp[i][0] != -99; i++) {
                    stones[temp[i][0]][temp[i][1]].setBackground(Color.red);
                }
            } else {
                return false;
            }
        } else // second click
        {
            if (ax == sourceX && ay == sourceY) {
                ...
                if ((sourceX + sourceY) % 2 == 0) {
                    stones[sourceX][sourceY].setBackground(Color.GRAY);
                } else {
                    stones[sourceX][sourceY].setBackground(Color.blue);
                }
                txtrError.setText(null);
                defaultButtonsBackground();
                return false;
